	...
	$.fn.uploadView = function(options){
		...
		$(this).each(function(i){
		    $(this).change(function(e){
		      handleFileSelect($(this), unit, width, height, allowType, maxSize, success);
		    });
		});
		
		var handleFileSelect = function(obj, _u, _w, _h, _type, _size, _callback){
			  ...
			  if(fileSize > maxSize){
			  	alert('上传图片超出允许上传大小,限制图片的大小在' + _size + 'M以内');
			  	return false;
			  }
			  ...
			  reader.onload = (function(theFile){
				      return function (e) {
						...
						var btn = label;
						var img = '<img src="' + tmpSrc + '"' + 'onclick="$(' + btn + ').click();" />';
						//consoleLog(reader, img);
						thisClosest.find(showBox).show().html(img);
						if (_w && _h) {
						  cssObj = { 'width':_w+_u, 'height':_h+_u };
						} else if (_w) {
						  cssObj = { 'width':_w+_u };
						} else if (_h) {
						  cssObj = { 'height':_h+_u };
